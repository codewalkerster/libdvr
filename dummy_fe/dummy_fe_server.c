...
static Dummy_TPInfo_t current_tp;
...
int64_t find_ts_offset_by_seek_time(FILE *fp, double seek_seconds) {
    if (!fp) return -1;

    uint8_t packet[TS_PACKET_SIZE];
    uint64_t offset = 0;
    uint64_t first_pcr = 0, target_pcr = 0;
    int pcr_pid = -1;
    int found_first_pcr = 0;

    // Find first PCR and PID
    while (fread(packet, 1, TS_PACKET_SIZE, fp) == TS_PACKET_SIZE) {
        if (packet[0] != 0x47) return -1;

        int pid = ((packet[1] & 0x1F) << 8) | packet[2];
        int afc = (packet[3] >> 4) & 0x03;

        if (pcr_pid != -1 && pid != pcr_pid) {
            continue;
        }

        if ((afc == 2 || afc == 3) && packet[4] >= 7 && (packet[5] & 0x10)) {
            uint64_t pcr = extract_pcr(packet);
            first_pcr = pcr;
            target_pcr = first_pcr + (uint64_t)(seek_seconds * 90000.0);
            INF("first_pcr: %llx, target_pcr: %llx\n", first_pcr, target_pcr);
            pcr_pid = pid;
            found_first_pcr = 1;
            break;
        }
    ...
    if (!found_first_pcr) return -1;

    // Seek to offset with PCR >= target_pcr
    fseek(fp, 0, SEEK_SET);
    offset = 0;
    ...
    while (fread(packet, 1, TS_PACKET_SIZE, fp) == TS_PACKET_SIZE) {
        ...
        if (pid == pcr_pid && (afc == 2 || afc == 3) &&
                packet[4] >= 7 && (packet[5] & 0x10)) {
            uint64_t pcr = extract_pcr(packet);
            if (pcr >= target_pcr) return offset;
        }
        offset += TS_PACKET_SIZE;
    }
...
void *dummy_inject_thread(void *arg)
{
    ...
    if (dummy_fe_info.seek_sec > 0) {
        int64_t offset = find_ts_offset_by_seek_time(file, dummy_fe_info.seek_sec);

        if (offset < 0) {
            ERR("seek failed, seek_sec: %d\n", dummy_fe_info.seek_sec);
        } else {
            fseek(file, offset, SEEK_SET);
            INF("seek to position: %llx\n", offset);
        }
    }

    while (dummy_fe_info.running) {
        uint64_t start_time;
        size_t data_size = DUMMY_BLOCK_SIZE;
                        ...
                        for (int i = 0; i < pcr_pid_count; i++) {
                            if (pcr_pids[i].pcr_pid ==  pid) {
                                ...
                                if (last_pcr != 0) {
                                    ...
                                    uint64_t elapsed_time_us = current_time - pcr_pids[i].start_time;
...
char* nonblocking_fgets(char* buf, size_t size) {
    static size_t pos = 0;
    static char internal_buf[1024];

    char ch;
    ssize_t n;

    while ((n = read(STDIN_FILENO, &ch, 1)) == 1) {
        if (ch == '\n' || pos >= sizeof(internal_buf) - 1) {
            internal_buf[pos] = '\0';
            strncpy(buf, internal_buf, size - 1);
            buf[size - 1] = '\0';
...
static struct option long_options[] = {
    ...
    {"dmxdevice", required_argument, 0, 'i'},
...
static void usage(char *argv[])
{
    ...
    INF("   -l --loop       Specify stream loop\n");
    INF("   -s --seek       Specify timestamp offset\n");
}
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt_long(argc, argv, "hf:d:i:l:s:", long_options, &opt_index)) != -1) {
        switch (opt) {
            case 'h':
                usage(argv);
                exit(0);
            ...
            case 'l':
                ...
                break;
            case 's':
                seek_sec = strtol(optarg, NULL, 10);
                INF("seek second: %d\n", seek_sec);
                break;
            default:
                usage(argv);
    ...
    memset(&current_tp, 0, sizeof(Dummy_TPInfo_t));
    ...
    dummy_fe_info.loop = loop;
    dummy_fe_info.seek_sec = seek_sec;
    ...
    while (running) {
        char* line = nonblocking_fgets(buf, sizeof(buf));
        if (line != NULL) {
            if (strncmp(line, "quit", 4) == 0) {
                INF("receive quit cmd, exit\n");
                running = 0;
                break;
            }
        }
        ...
        if (prop.data == DUMMY_FE_INIT || prop.data == DUMMY_FE_INVALID) {
            usleep(50*1000);
            continue;
        }
        if ((prop.data == DUMMY_FE_LOCKED || prop.data == DUMMY_FE_UNLOCKED)
            && !dummy_fe_info.running) {
            INF("dummy fe state: %d\n", prop.data);
            INF("try to re-lock\n");
        } else if (prop.data != DUMMY_FE_LOCK){
            continue;
        }
        /*else {
        ...
        if (!(current_tp.freqM == freqM && current_tp.delivery_system == delivery_system)) {
        ...
        current_tp.freqM = freqM;
        current_tp.delivery_system = delivery_system;
